...
{
    glDeleteBuffers(buffer_count(), &meanBuffer);
}
...
GaussianData::GaussianData(int num_gaussians, float* mean_data, float* rot_data, float* scale_data, float* alpha_data, float* color_data)
{
    ...
    glCreateBuffers(buffer_count(), &meanBuffer);
    set_buffers(num_gaussians, mean_data, rot_data, scale_data, alpha_data, color_data);
}
...
void GaussianData::set_buffers(
    int num_gaussians, float* mean_data, float* rot_data, float* scale_data, float* alpha_data, float* color_data) const
{
    glNamedBufferData(meanBuffer, num_gaussians * 3 * sizeof(float), mean_data, GL_STATIC_DRAW);
    glNamedBufferData(rotBuffer, num_gaussians * 4 * sizeof(float), rot_data, GL_STATIC_DRAW);
    glNamedBufferData(scaleBuffer, num_gaussians * 3 * sizeof(float), scale_data, GL_STATIC_DRAW);
    glNamedBufferData(alphaBuffer, num_gaussians * sizeof(float), alpha_data, GL_STATIC_DRAW);
    glNamedBufferData(colorBuffer, num_gaussians * sizeof(float) * 48, color_data, GL_STATIC_DRAW);
    CHECK_GL_ERROR;
}
